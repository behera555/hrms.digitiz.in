    ...
    <div class="side-app main-container">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form method="post" id="notice_board_edit" action="" enctype="multipart/form-data">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" value="{{$notice_board->notice_heading}}" placeholder="{{ __('Notice Heading') }}" name="{{ __('notice_heading') }}">
                                    </div>
                                    <div class="form-group">
                                        <div class="custom-controls-stacked d-md-flex">
                                            <label class="custom-control custom-radio success me-4">
                                                <input type="radio" class="custom-control-input" id="show" name="type"
                                                    value="employees" {{ ($notice_board->type=="employees")? "checked" : "" }}>
                                            ...
                                            <label class="custom-control custom-radio success">
                                                <input type="radio"  class="custom-control-input" id="hide" name="type"
                                                    value="clients" {{ ($notice_board->type=="clients")? "checked" : "" }}>
                                                <span class="custom-control-label">{{ __('To Clients') }}</span>
                                            </label>
                                        </div>
                                        @if($notice_board->department != NUll && $notice_board->type =="clients")
                                        ...
                                        <p><label class="form-label">{{ __('Select Employee') }}:</label></p>
                                            ...
                                            <option value="{{$val->emp_id}}"  {{ $val->emp_id == $notice_board->department ? 'selected' : '' }}>{{$val->name}}</option>
                                          @endforeach
                                        </select>
                                        </p>
                                        @endif
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea name="notice_details" class="form-control content" placeholder="{{ __('Notice Details') }}">{{ $notice_board->notice_details }}</textarea>
                                    </div>
                                </div>
...
@section('script')
        ...
        $("#notice_board_edit").submit(function() {
            event.preventDefault();
            $("#submit").prop('disabled', true);
            $("#submit").html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Loading....');
            axios.post("{{ route('notice-board-edit', $notice_board->id) }}", new FormData($("#notice_board_edit")[0])).then(response => {
                var data = response.data;
                $('#notice_board_edit')[0].reset();
                if (data.success) notif({
                    msg: "<b><i class='fa fa-check-circle-o fs-20 me-2'></i></b> Notice Board Updated Successfully",
                    type: "success"
                }, setTimeout(function() { location.replace("{{ route('notice-board') }}");  }, 3000));
                else {
