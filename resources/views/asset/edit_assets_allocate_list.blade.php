@extends('layouts.app')
@section('title', 'Edit Asset Allocate')
@section('content')
    ...
    <div class="side-app main-container">
        ...
        <div class="page-header d-xl-flex d-block">
            <div class="page-leftheader">
                <div class="page-title">{{ __('Edit Asset Allocate') }}</div>
            </div>
                                    ...
                                    <div class="col-md-3">
                                        <div class="input-group">
                                            ...
                                            <select class="form-control select2-show-search custom-select"
                                                        ...
                                                        <option value="{{$val->asset_name}}"  {{ $val->asset_name == $assets_allocate_to->asset_name ? 'selected' : '' }}>{{$val->asset_name}}</option>
                                                        @endforeach
                                                    </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="input-group">
                                            <div class="input-group-prepend">
                                                ...
                                                <select class="form-control select2-show-search custom-select"
                                                        ...
                                                        <option value="{{$val->emp_id}}"  {{ $val->emp_id == $assets_allocate_to->employee_name ? 'selected' : '' }}>{{$val->display_name}}</option>
                                                        @endforeach
                                                    </select>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control fc-datepicker" value="{{$assets_allocate_to->allocate_date}}" name="allocate_date" 
...
@section('script')
        ...
        $("#assets_allocate_post").submit(function() {
            ...
            axios.post("{{ route('assets-allocate-to-edit', $assets_allocate_to->id) }}", new FormData($("#assets_allocate_post")[0])).then(
                response => {
                    var data = response.data;
                    $('#assets_allocate_post')[0].reset();
                    if (data.success) notif({
                        msg: "<b><i class='fa fa-check-circle-o fs-20 me-2'></i></b>Assets Allocate Updated Successfully",
                        type: "success"
                    }, setTimeout(function() {
