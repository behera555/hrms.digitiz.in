@extends('layouts.app')
@section('title', 'Employee Reliving Letter')
@section('content')
    ...
    <div class="side-app main-container">
        ...
        <div class="page-header d-xl-flex d-block">
            <div class="page-leftheader">
                <div class="page-title">Employees Reliving Letter</div>
            </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form method="post" id="reliving_letter" action="" enctype="multipart/form-data">
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <label class="form-label mb-0 mt-2">{{ __('Resignation Date') }}</label>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="input-group">
                                            ...
                                            </div><input name="resignation_email_dated" class="form-control" data-bs-toggle="modaldatepicker" placeholder="MM/DD/YYYY" type="text">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="row">
                                    <div class="col-md-3">
                                        <label class="form-label mb-0 mt-2">{{ __('Relieved Date') }}</label>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="input-group">
                                            <div class="input-group-prepend">
                                            ...
                                            </div><input name="relieved_date" class="form-control" data-bs-toggle="modaldatepicker" placeholder="MM/DD/YYYY" type="text">
                                        </div>
                                    </div>
                                </div>
                            </div>
       ...
       <div class="row">
							<div class="col-xl-12 col-md-12 col-lg-12">
								<div class="card">
									<div class="card-header  border-0">
										<h4 class="card-title">Reliving Letter Summary</h4>
									</div>
									<div class="card-body">
											...
											<table class="table  table-vcenter text-nowrap table-bordered border-bottom" id="emp-attendance">
												<thead>
													<tr>
														...
														<th class="border-bottom-0">Employee Joining Date</th>
														<th class="border-bottom-0"> Resignation Email Date</th>
														<th class="border-bottom-0"> Relieved Date</th>
														<th class="border-bottom-0">Action</th>
													</tr>
												</thead>
												<tbody>
												    @foreach($employees_relieving_letter as $key => $val)
													<tr>
														<td>{{ ($employees_relieving_letter->currentpage()-1) * $employees_relieving_letter->perpage() + $key + 1 }}</td>
														<td>{{ $val->employee_name }}</td>
														<td>{{ $val->emp_id }}</td>
														<td>{{ $val->designation }}</td>
														<td class="font-weight-semibold">{{ $val->date_of_joining }}</td>
											 ...
											 <nav aria-label="Page navigation">{!! $employees_relieving_letter->links('pagination.custom') !!}</nav>
										</div>
									</div>
								</div>
							</div>
        ...
        $("#reliving_letter").submit(function() {
            event.preventDefault();
            $("#submit").prop('disabled', true);
            $("#submit").html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>Loading....');
            axios.post("{{ route('employees-reliving-letter') }}", new FormData($("#reliving_letter")[0])).then(response => {
                var data = response.data;
                if (data.success) 
                notif({msg: "<b><i class='fa fa-check-circle-o fs-20 me-2'></i></b>  Reliving Letter Submitted Successfully",
                    type: "success"}, setTimeout(function() { location.replace("{{ route('employees-reliving-letter') }}"); }, 3000));
                else {
...
function validate(id, title) {
    swal({
        ...
        $.ajax({
            type: 'DELETE',
            url: "{{ route('employees-reliving-letter-destroy') }}" + '/' + id,
